...
(function(){
  ...
  // ----- UI Card -----
  // Wait for DOM to be ready, then find the right container
  function initMemristorLab() {
    const container = document.querySelector("#memristor-container");
    const fallbackHost = document.querySelector("#gallery, main, .cards, .container") || document.body;

    console.log('Memristor Lab initializing...');
    console.log('Container found:', container);
    console.log('Fallback host:', fallbackHost);

    const host = container || fallbackHost;

    if (!host) {
      console.error('No suitable host found for Memristor Lab!');
      return;
    }

    console.log('Using host:', host.id || host.tagName);
    ...
    console.log('Memristor Pathfinder initialization complete');
  }

  // Start the initialization when DOM is ready
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initMemristorLab);
  } else {
    initMemristorLab();
  }
})();
